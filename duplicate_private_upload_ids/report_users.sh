...
report_users()
{
  users="$(mongo ${MONGO_OPTIONS} ${USERS_DATABASE} --eval "db.users.find(${USER_CLAUSE}).forEach(function(f) { print(f.username + '|' + f.userid + '|' + f.private.meta.id + '|' + f.private.meta.hash); })")"
